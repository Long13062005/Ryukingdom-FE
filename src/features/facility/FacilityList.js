...
export function FacilityList() {
    ...
    const fetchFacility = async (page,size,nameSearch) => {
        try {
            const data = await FacilityService.getFacilityList(localStorage.getItem("token"),page,size,nameSearch);
            setTotalPages(data.totalPages);
            setFacilitys(data.content);
        } catch (error) {
...
const handleSearch = async (values) => {
    try {
        ...
        if (nameSearch === "") {
            await fetchFacility(0, size);
            return;
        }
        const data = await FacilityService.getFacilityList(localStorage.getItem("token"), 0, 4, nameSearch);
            setTotalPages(data.totalPages);
            setFacilitys(data.content);
    } catch (error) {
        await Swal.fire({
    ...
    useEffect(() => {
        fetchFacility(page,size,nameSearch);
                        ...
                        <div className="card-body" style={{background: "#171821"}}>
                            <div className="table-responsive">
                                <table className="table table-striped table-hover">
                                    ...
                                    <tbody>
                                    {facilitys.length > 0 ? (facilitys.map((facility, index) => (<tr key={facility.id}>
                                                <td>
                                                    <img
                                                        src={facility.imgUrl == null || facility.imgUrl === '' ? 'https://firebasestorage.googleapis.com/v0/b/ryukingdom-48b31.appspot.com/o/avatar.webp?alt=media&token=d95b5759-3456-4c46-9d78-4766f9512868' : facility.imgUrl}
                                                        alt={facility.name}
                                                ...
                                                <td className="text-center">
                                                    ...
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em"
                                                         ...
                                                         onClick={() => updateForm(facility.id)}>
                                                        <path
